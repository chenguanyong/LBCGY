...
function dowork1(){
	...
	if(check_html($xmlobj['un1']['data'])&&($xmlobj['un1']['data']=="")){
		//error_dump($str);
		write_json::create_massage(array("phone","bucunzai","string","phone","1","1"));
		exit();
		...
		//error_dump($str);
		write_json::create_massage(array("phone","bucunzai","string","1","1","1"));
		exit();
...
$xml=new massage_xml();
	...
	$shenqing=$shenqing.$xml->prase_xml1(array("name"=>"phone","data"=>'fgsgdsfg',"phone"=>$xmlobj['un2']['data'],"old"=>$xmlobj['un1']['data']),"body");
