...
$time=@session_data::getsession(session_id(),'c33');
	if(isset($time))
	{
		if(time()-$time>3600){
...
class  updatedata extends Thread{
	...
	$shenqing=$shenqing.$xml->prase_xml1(array("name"=>"dingdan","data"=>$this->username,"type"=>$this->type),"body");
...
// [order_num],[shop_name],[order_state],[order_cash],[createtime],[shop_num],[shop_attr]
...
//print_r($this->data_array);
	...
	FOREACH($this->data_array as $rtt){
		...
		$m->insert1(array("insert into temp_order ",$rtt));
	}
