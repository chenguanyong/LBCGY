...
$time=@session_data::getsession(session_id(),'b1');
if(isset($time))
	{
		if(time()-$time>3600){
		...
		$_SESSION['a3']=time();
			...
			$page=$_POST['page'];
			$page=((int)$page)*15;
			$rr=new mythread($type,$page,$pici);
			$rr->start();
			$_SESSION['a3']=time();
		}
...
class mythread extends Thread{
	...
	private $pici;
	private $page;
	public function __construct($type,$page,$pici){
        $this->type=$type;
		$this->page=$page;
		$this->pici=$pici;
...
$result=$mysql->read("SELECT `num`,`createtime`,`link`,`id1`,`name`  FROM `linbei_ad`.`tuijian` 
 
 where 'type'=".$this->type." pici =".$this->pici." LIMIT ".$this->page.", 15 ;");
...
class  updatedata extends Thread{
	...
	$shenqing=$shenqing.$xml->prase_xml1(array("name"=>"tj","name1"=>"initmore2","data"=>"is","type"=>$this->type),"body");
...
// echo  "sdfs1";
 ...
 //print_r($arry);
    ...
    if($arry['un0']['data00']=="kong"){
     $arry['hand']=array("size"=>count($arry));
    	$xml=new write_json();
		$xml->name=$arry;
		$xml->prase_json1();
		echo $xml->getresult();
